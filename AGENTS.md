# AGENTS.md — Vibe Code Launchpad

> **This is the canonical AI context file for this project.** Copies exist at `CLAUDE.md` (for Claude Code) and `.cursorrules` (for Cursor) to ensure your AI coding tool picks it up automatically. If you update this guidance, update those files too.
...
│
└── .claude/                   # AI assistant tools (Claude Code skills)
...
If the student used "Use this template" on GitHub, they already have a repo. Otherwise:
...
3. **Push code to GitHub** (if not already there):
   ```bash
   git init
   git add .
...
4. **Connect to Netlify:**
   ...
   - Click "Deploy"
5. **Set environment variables on Netlify** — this is the step people forget!
   - Go to **Site configuration > Environment variables**
   - Add `GEMINI_API_KEY` with the real API key value
   - Trigger a redeploy
   - Without this, the site will load but AI features will fail
...
**If using Claude Code, use the `/add-auth` skill for a guided walkthrough.**
